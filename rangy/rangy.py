...
def _parse(self, rangy) -> Tuple[Union[int, float], Union[int, float]]:
    ...
    Returns:
        A tuple (min_count, max_count) representing the parsed count range.  `INFINITY` is used for unbounded maximums.
...
class Rangy:
    """
    Represents a flexible rangy specification, encompassing exact values, ranges, and unbounded rangys.
    ...
    3. **Unbounded:** A rangy representing any non-negative integer (*), or any positive integer (+).

    Internally, a `Rangy` stores the minimum and maximum allowed rangys. For exact rangys, the minimum and maximum are equal. For unbounded rangys, the maximum is represented by a large internal constant (`INFINITY`).
